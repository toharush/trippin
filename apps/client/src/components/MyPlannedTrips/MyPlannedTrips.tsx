...
export default function MyPlannedTrips({ onBack }: props) {
  ...
  const [mappedTripsWithDisplayName, setMappedTripsWithDisplayName] = useState<
    PlannedTrip[]
  >([]);
  const [orderedTrips, setOrderedTrips] = useState<PlannedTrip[]>([]);
  const [isPlannedTripDetailsOpen, setIsPlannedTripDetailsOpen] = useState(
    false
  );
  const [lastCalculatedTripId, setLastCalculatedTripId] = useState<
    number | null
  >(null);
  ...
  const mapTripsToPlannedTrips = async (
    trips: ITrip[] | null
  ): Promise<PlannedTrip[]> => {
    if (!trips) return [];
    ...
    for (const trip of trips) {
      ...
      const subtitle = formatTripSubtitle(
        new Date(trip.start_date),
        new Date(trip.end_date)
      );
  ...
  const handleBackToAllTrips = () => {
    lastCalculatedTripId
      ? SetSelectedTripId(lastCalculatedTripId)
      : ResetSelectedTripId();
          ...
          {loading ? (
            ...
            <>
              <div className="my-planned-trips">
                <div className="my-planned-trips-header">My Planned Trips</div>
                <ImageList
                  sx={{ maxHeight: "70vh", padding: "1vh" }}
                  cols={2}
                  gap={10}
                  rowHeight={"auto"}
                >
                  {orderedTrips.map((trip: PlannedTrip) => (
                    <ImageListItem key={trip.id}>
                      ...
                      <ImageListItemBar
                        ...
                        title={
                          <span className="image-list-item-bar-title">
                            {trip.title}
                          </span>
                        }
                        subtitle={
                          <span className="image-list-item-bar-subtitle">
                            {trip.subtitle}
                          </span>
                        }
                        position="top"
