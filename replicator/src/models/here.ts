import client, { QueryResult } from "../utils/database/client";
...
export const insert_category = async(name: string): Promise<number> => {
    return await (await query(`INSERT INTO ${schema}.${TABLES.CATEGORY}(name) VALUES ($1) RETURNING id`, [name])).rows[0]?.id;
}

export const get_category_by_name = async(name: string): Promise<number> => {
    const res = await (await query(`SELECT id FROM ${schema}.${TABLES.CATEGORY} WHERE LOWER(name) = LOWER($1)`, [name]));
...
export const insert_position = async(lat: number, lng: number): Promise<number>  => {
    return await (await query(`INSERT INTO ${schema}.${TABLES.POSITION}(lat, lng) VALUES ($1, $2) RETURNING id`, [lat, lng])).rows[0]?.id;
}

export const get_position_id = async(lat: number, lng: number): Promise<number> => {
    const res = await (await query(`SELECT id FROM ${schema}.${TABLES.POSITION} WHERE lat=$1 AND lng=$2`, [lat, lng]));
...
export const get_places = async(limit: number, offset: number): Promise<{id: string, addressId: number, label: string}[]> => {
    return await (await query(`SELECT id, "addressId", (SELECT "label" FROM ${schema}.${TABLES.ADDRESS} WHERE address."id" = "addressId") as label FROM ${schema}.${TABLES.PLACE} ORDER BY place."updated_at" LIMIT $1 OFFSET $2;`, [limit, offset])).rows;
}
